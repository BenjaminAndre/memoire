Cette section décrit les automates finis, fait le lien avec la notion de langage régulier et propose une représentation visuelle de ces automates. La notation suivie dans cette section s'appuye principalement sur \cite{Hopcroft79}, \cite{Hopcroft00} et \cite{Kozen97}.
...
Soit la transition $\delta(q,a)=p$ (dans un ADF). Pour $q$, c'est une \emph{transition sortante sur a}. Pour $p$, c'est une \emph{transition entrante sur a}. On peut aussi dire que $p$ est \emph{atteignable}. Si au contraire aucune transition de la forme $\delta(q,a)=p$ n'existe et que $p$ n'est pas l'état initial, alors $p$ est \emph{inatteignable}.

Si $\delta(q,a)=P=\{p_1,p_2,\dots,p_n\}$ dans un ANF, alors les états $\{p_1,p_2,\dots,p_n\}$ auront une transition entrante sur $a$.

Dans le cas des ANF et $\epsilon$-ANF, il peut être pratique d'utiliser $\delta$ sur un ensemble d'états $S$. A ce moment, $\delta(S,a)=\bigcup_{q\in S}\delta(q,a)$ avec $a\in \Sigma$.
...
\begin{itemize}
  \item Les en-têtes de colonnes sont des symboles appartenant à $\Sigma$.
...
\begin{example}[Automate non-déterministe fini avec transitions sur $\epsilon$]\label{ex:anf}
...
\begin{example}[Graphe d'automate]
 ...
 \begin{minipage}[t]{0.5\textwidth}
   \begin{figure}[H]
   	...
   	(B) edge node{b} (C);
   	\end{tikzpicture}
   	\caption{Graphe du $\Sigma$-ANF de la table \ref{table:eanfdelta}}\label{fig:eanf}
   \end{figure}
\end{minipage}
\end{example}
...
Cette sous-section introduit l'algorithme ECLOSE. Cet algorithme concerne les $\epsilon-ANF$. Ilpermet, à partir d'un état spécifique, de calculer l'ensemble des états atteignables uniquement par des transitions sur $\epsilon$. Ce calcul sert notemment au test d'appartenance d'un mot à un $\epsilon$-ANF.
...
Le \emph{langage représenté} par un automate \automaton peut alors se définir comme les mots qui, par l'application de $\hdelta$ sur l'état initial, donnent un état acceptant. Si deux automates représentent le même langage, ils sont dit \emph{équivalents}. Voici les définitions ensemblistes, respectivement pour un ADF et pour un $(\epsilon)$-ANF
$$
L(A)= \{w \in \Sigma^* | \hdelta(q_0,w) \in F\} \hspace{0.5cm}
L(A)= \{w \in \Sigma^* | \exists q \in \hdelta(q_0,w), q \in F\}
$$
Ainsi, un mot $w$ appartient à un langage $L$ représenté par l'automate $A$ si $\hdelta(q_0,w) \in F$.
...
La complexité de l'algorithme \ref{alg:membership} dépend du type d'automate considéré. Considérons que $|w|=n$.
...
L'opération 14 se fait en $\mathcal{O}(\log_2(n))$ pour un ADF et $\mathcal{O}(n\log_2(n))$ pour un ANF ou $\Sigma$-ANF. Cette valeur suppose une recherche efficace dans F (en $\mathcal{O}(\log_2(n))$).
...
Pouvoir convertir une expression réguliere en automate permet d'exécuter cet automate sur un machine pour tester l'appartenance. Ainsi, grâce à cette méthode, une grande classe de problèmes peut être résolue.
...
\begin{proof}
  ...
  Une possibilité est $i \neq j$. Alors les chemins possibles ne se composent que d'un arc allant directement de $i$ à $j$. Pour les construire :
	...
	\begin{itemize}
		\item Il n'existe pas de symbole $a$ tel que $\delta(i,a)=j$. Alors, $R_{ij}^0=\emptyset$
		\item Il existe un unique symbole $a$ tel que  $\delta(i,a)=j$. Alors, $R_{ij}^0=a$
		\item Il existe des symboles $a_1,a_2,\dots,a_k$ tels que $\forall l \in \{1,\dots, k\}, \delta(i,a_l)=j$. Alors, $R_{ij}^0=a_1+a_2+\dots+a_k$
	\end{itemize}
...
\stepcounter{algo}
\begin{complexity}
	Soit un ADF \automaton comportant $n$ états. Pour connaître la complexité totale de cet algorithme, il faut connaître le nombre total d'expression régulières construites et la longueur de chacune de celles-ci.
...
\begin{proof}
  ...
  Intuitivement, ces invariants servent à assurer que l'automate ainsi créé est lu de gauche à droite tel une expression régulière.
	...
	\begin{itemize}
		...
		\item L'expression régulière est de forme $E^*$. Par induction, il existe un automate exprimant le même langage que celui exprimé par $E$. L'automate pour $E^*$ est construit comme en (a). Un arc $\epsilon$ entrant en $E$ permet de concaténer plusieurs chemins dans $E$, donnant des mots des langages décrits par $E^1$,$E^2$,$E^3$,... Le tout étant l'ensemble des mots possibles $L(E)^*$. On a bien $L(E^*)=L(E)^*$.
		\item L'expression régulière est de forme $EF$. Par induction, il existe des automates représentants les mêmes langages que $E$ et $F$ et respectant notre invariant. L'automate abstrait (b) représente cette concaténation. En effet, un mot de cet automate doit se composer d'un mot $v\in L(E)$ et d'un mot $w \in L(F)$. Les mots possibles sont alors de la forme $v\epsilon w$. Donc (b) construit bien une expression régulière $L(EF)=L(E)L(F)$.
		\item L'expression régulière est de forme $E+F$. Alors, comme mis en évidence par l'automate abstrait (c), il existe des automates correspondants aux expressions $E$ et $F$. Cette construction, en particulier les transitions sur $\epsilon$, permet à (c) de représenter tout mot de $L(E)$ ou $L(F)$. Le langage est alors en concordance avec la définition d'une expression régulière $L(E+F)=L(E)\cup L(F)$.
	\end{itemize}
...
\stepcounter{algo}
\begin{complexity}
	Soit une expression régulière $E$ contenant $n$ symboles (alphabet et opérations comprises) représentant un langage $L$. Si un arbre syntaxique est créé pour $E$, il est possible de construire un ANF pour $L$ en $\mathcal{O}(n)$. En effet :
	\begin{itemize}
    ...
    Cependant, chacune des opérations implique au plus la création de 2 états et 4 arcs au plus. La récurrence s'effectue en $\mathcal{O}(n*6)=\mathcal{O}(n)$
	\end{itemize}
...
Cette section présente une méthode permettant de créer un ADF à partir d'un ANF et réciproquement. Ici, ANF est considéré au sens-large et peut tout aussi bien être un ANF normal qu'un $\Sigma$-ANF. Ceci permet de justifier l'abstraction faite entre les différents types d'automates finis et l'utilisation du même terme pour tous ceux-ci.
...
est défini par :
\begin{itemize}
	\item $Q_D = \{S | S \subseteq Q \text{ et } S \text{ est fermé sur epsilon}\}$. Concrètement, $Q_D$ est l'ensemble des parties des $Q$ fermées sur $\epsilon$. Cette fermeture s'écrit $S$=ECLOSE(S), ce qui signifie que chaque transition sur $\epsilon$ depuis un état de $S$ mène à un état également dans $S$. L'ensemble $\emptyset$ est fermé sur $\epsilon$.
