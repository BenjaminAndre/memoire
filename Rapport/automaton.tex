...
\end{example}
 ...
 Cette représentation d'un automate peut sembler plus naturelle pour un humain alors que la table de transitions est plus proche d'un langage informatique.
...
Si deux automates représentent le même langage, ils sont dit \emph{équivalents}.
...
\end{theorem}
\begin{proof}
	Supposons qu'il existe un ADF \automaton tel que $L(A)=L$. $Q$ étant un ensemble fini, on peut définir sa cardinalité : $|Q|=n$. Supposons que ses états soient nommés $\{1,2,\dots,n\}$. Il est possible de construire des expressions régulières par induction sur le nombre d'états considérés. De plus, un tel automate est aisément représenté dans un ordre séquentiel, de gauche à droite. Ceci permet de séparer visuellement les $k$ premiers états du reste. Par abus de langage, les nombres et états désignés sont utilisés indifféremment. Celà ne doit pas entraîner de confusion car chaque nombre correspond exactement à un état.

  Soient $i,j,k \in Q$, tous équivalents à des nombres inférieurs à $n$. Définissons $E_{ij}^k$ comme étant l'expression régulière décrivant un langage constitué des mots $w$ tels que $\delta^*(i,w)=j$ et qu'aucun état intermédiaire n'ait un nombre supérieur à $k$. Il n'y a pas de contrainte sur $i$ et $j$.
	...
	L'exemple ci-dessus illustre ce fait qu'aucun état supérieur à $k$ ne peut faire partie des intermédiaires. Intuitivement, il s'agit d'un automate auquel on a enlevé les transitions :
  \begin{itemize}
  ...
  \end{itemize}
  En pratiques, celles-ci ne sont juste pas autorisées.
...
\begin{complexity}[Conversion ANF vers ADF]
	...
	Le détail est donné sur $s$ car, comme mentionné par \cite{Hopcroft00}, en pratique le nombre d'états dans l'ADF obtenu est rarement de l'ordre de $2^n$, typiquement de l'ordre de $n$. Dans ce cas là, la complexité devient $\mathcal{O}(n^2)$.
